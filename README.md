...
Use a fresh batch collected with extension version 0.5.1 or newer so the locked Source Page Context includes both the top Chrome document and the candidate-bearing recommendation frame. Native Favorite success requires exact Platform Identity confirmation in the Boss favorite-management experience. A safe explicit failure may retry once; unknown and platform-restriction outcomes never auto-retry. If the source document is interrupted, use the extension's interruption-recovery control after the lease expires; it leaves unknown tasks terminal and only unlocks remaining pending tasks. This version does not refresh the recommendation page, advance to another Capture Batch, contact candidates, or cancel favorites.
